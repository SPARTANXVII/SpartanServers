...
{	
	...
	override void OnStartClient(ActionData action_data)
	{
		...
		if (action_data.m_MainItem.IsInherited(Chainsaw) && !GetGame().IsDedicatedServer()) {
	...
	override void OnFinishProgressServer(ActionData action_data)
	{
		...
		WoodenPlank planks = WoodenPlank.Cast(GetGame().CreateObjectEx("WoodenPlank", action_data.m_Player.GetPosition(), ECE_PLACE_ON_SURFACE) );
		...
		if ((planks.GetQuantity() + Chainsaw.PLANKS_FROM_TREES) >= planks.GetQuantityMax()) {
			...
			if (remnant > 0) {
				planks = WoodenPlank.Cast(GetGame().CreateObjectEx("WoodenPlank", action_data.m_Player.GetPosition(), ECE_PLACE_ON_SURFACE));
