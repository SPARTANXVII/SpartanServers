...
class ActionChainsawTree: ActionContinuousBase
{
	...
	override void OnFinishProgressServer(ActionData action_data)
	{
		...
		ItemBase planks = ItemBase.Cast(GetGame().CreateObject("WoodenPlank",action_data.m_Player.GetPosition(), false));
		planks.SetQuantity(Chainsaw.PLANKS_FROM_TREES);
		for (int i = 0; i < Chainsaw.LOGS_FROM_TREES; i++) {
			GetGame().CreateObject("WoodenLog", action_data.m_Player.GetPosition(), false);
		}
