...
class Zen_CombatLogFlare extends Roadflare
{
	...
	override void EEInit()
	{
		...
		if (SERVER_RESTART)
		{
			GetGame().GetCallQueue(CALL_CATEGORY_SYSTEM).CallLater(DeleteFlare, 2000, false);
			return;
		}
		#endif

		// 1 second after creation, ignite flare
		GetGame().GetCallQueue(CALL_CATEGORY_SYSTEM).CallLater(IgniteCombatFlare, 1000, false);
	}
